...
namespace PuhPTests
{
	...
	class Random
	{
		...
		public:
			 ...
			 * Leave at 0 to do random seeding with random_device.
			 * 
			 * inline required to avoid compilation errors
			 */
			static inline unsigned int fixed_seed = 0;
			
			//
			Random()
			{
				if ( Random::fixed_seed == 0 ) {
					this->generator_ = std::mt19937_64(random_device());
				}
				else {
					this->generator_ = std::mt19937_64(Random::fixed_seed);
				}
			}
			...
			//
			std::vector<int> get(int min, int max, size_t count)
			{
				std::vector<int> v;
				
				//
				for ( size_t i = 0; i < count; i++ ) {
					v.push_back(this->get(min, max));
				}
