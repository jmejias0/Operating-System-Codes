...
namespace PuhPTests
{
	...
	class DeferredAssertion
	{
		...
		public:
			...
			DeferredAssertion
			(
				std::string label, int points, AssertionType assertionType,
			...
			///	CTOR with lambda and one exception type
			DeferredAssertion
			(
				std::string label, int points, AssertionType assertionType,
				std::function<void()> func, const std::type_info& exception_type
			)
			{
				//
				this->label_ = label;
				...
				//	Assert the Lambda throws a specific type of exception
				if ( this->assertionType_ == AssertionType::TypedException ) {
					this->lambda_ = [func, &exception_type](std::string& out_message) -> bool
					{
						return Assertions::assertTypedException(func, exception_type, out_message);
					};
				}
				...
				//	Unsupported
					...
					(
						"DeferredAssertion() does not support type "
						+ Assertions::typeToString(this->assertionType_)
						+ " with a lambda and one value"
					);
				}
			}
