	...
	class Test
	{
		...
		public:
			...
			(
				...
				int points = 0,
				std::string label = Test::DEFAULT_LABEL
			)
			{
				this->adjustPointsPossible(points, label);
			...
			/**
			 * Attempts to run the passed lambda "func",
			 * and grants points if an exception of a specific type
			 * DOES OCCUR.
			 */
			void assertTypedException
			(
				std::function<void()> func,
			...
			nlohmann::json toGradescopeJson()
			{
				return nlohmann::json
				{
					{
						"score",
						this->isUsingNormalizedPoints()
							?
							this->computeNormalizedPoints()
							:
							this->getPoints()
					},
					{
						"max_score",
						isUsingNormalizedPoints()
							?
							this->getNormalizedPointsPossibleTarget()
							:
							this->getComputedPointsPossible()
					},
