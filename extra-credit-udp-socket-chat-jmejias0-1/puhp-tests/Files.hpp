...
namespace PuhPTests
{
	...
	class Files
	{
		...
		public:
			...
			static bool removeFile(std::string file_name, bool throw_on_error = false)
			{
				bool returnValue = false;
				int removeResult = remove( file_name.c_str() );
				if ( removeResult == 0 ) {
					returnValue = true;
				}
				else if ( throw_on_error ) {
					throw std::runtime_error(std::string("Unable to remove file: ") + file_name);
				}
