...
namespace PuhPTests
{
	...
	class Assertions
	{
		...
		public:
			...
			static std::string typeToString(AssertionType t)
			{
				switch(t)
				{
					...
					case AssertionType::Exception:
						return "Exception";
					case AssertionType::TypedException:
			 ...
			 * Returns "true" if any exception was encountered, false otherwise
			 */
			...
			/**
			 * Attempts to run the passed lambda "func".
			 * Returns "true" if the correct exception type was encountered,
			 * or false otherwise
			 */
			static bool assertTypedException
			(
			...
			)
			{
				...
				{
					if ( typeid(e).hash_code() == exception_type.hash_code() ) {
						
						caught_correct_exception = true;
						
						out_message =
							std::string("Caught correct exception (") + typeid(e).name() + ") :: "
							+ e.what()
							;
					}
					else {
						...
						out_message = 
							std::string("Caught exception, but not the right type.")
							+ " Expected [" + exception_type.name() + "]"
							+ " but caught [" + typeid(e).name() + "]."
							+ " Message was :: " + e.what()
							;
					}
				}
				...
				{
					out_message = "Caught unexpected exception type!";
				}
				
				return caught_correct_exception;
			}
